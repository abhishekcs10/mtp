#This script is to allocate the class of priorities as in linux (-19 to +20 i.e. 40 priority levels) equally among the available cores. Then distribute all threads among the set of cores that will handle same class of priority
...
last_10_pro=`ps -axo pid,ni  |  awk '{print $1 " " $2}'` #this is to extract pids
...
#group_processor define how many cores will be handling a set of priorities
group_processor=2
logical_num_cores=`python -c "from math import ceil; print (ceil($num_cores/$group_processor))"`
...
'
	...
	assign_num_niceness=`python -c "from math import ceil; print (ceil($logical_num_cores/$init_nice))"`
	echo $assign_num_niceness
	for (( i=0; i<${num_cores}; i=i+${group_processor} ));
	do
